...
serve(async (req) => {
  ...
  const rateLimitResult = await checkRateLimitWithLogging(identifier, 'suggest-date-ideas', RATE_LIMITS.AI_FUNCTION, req);
  if (!rateLimitResult.allowed) {
    console.log(`🚫 SUGGEST-IDEAS: Rate limit ${rateLimitResult.count}/${rateLimitResult.limit}`);
