 ...
 * Searches for restaurants, cafes, bars, entertainment, and activity venues.
 */
...
function buildOverpassQuery(
  ...
  const r = Math.min(radius, 50000); // Cap at 50km
  const parts: string[] = [];

  // Always search for restaurants, cafes, bars
  ...
  return `[out:json][timeout:25];(${parts.join('')});out center body ${limit};`;
}

function extractCuisineType(tags: Record<string, string>): string {
  if (tags.cuisine) {
    // OSM cuisine can be semicolon-separated
...
function extractPriceRange(tags: Record<string, string>): string {
  // OSM doesn't have a standard price field, but some mappers use it
...
function extractTags(tags: Record<string, string>): string[] {
  const result: string[] = [];
  if (tags.cuisine) {
    result.push(...tags.cuisine.split(';').map(c => c.trim().replace(/_/g, ' ')));
  }
...
serve(async (req) => {
  ...
  if (!rateLimitResult.allowed) {
    console.log(`🚫 OVERPASS: Rate limit ${rateLimitResult.count}/${rateLimitResult.limit}`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('🔴 OVERPASS: Query failed', response.status, errorText);
    ...
    // Filter out excluded types and venues without names
    ...
    if (osmCuisineValues.length > 0) {
      ...
      // If we got enough cuisine matches, use them; otherwise include all
      if (cuisineMatched.length >= 3) {
        matched = cuisineMatched;
      }
    }
    ...
    for (const venue of venues) {
      ...
      if (error) {
        console.error('🔴 OVERPASS: Error saving venue', venue.name, error.message);
      }
    }

    console.log('✅ OVERPASS: Saved', venues.length, 'venues to database');
    ...
    return new Response(
      JSON.stringify({ error: msg, venues: [], count: 0 }),
